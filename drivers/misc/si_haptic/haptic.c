	...
	if (sih_haptic->rtp.rtp_file_num == AW8697_WAVEFORM_INDEX_OLD_STEADY) {
		if (DEVICE_ID_0815 == sih_haptic->device_id) {
			ret = request_firmware(&rtp_file,
					...
					sih_haptic->dev);
			hp_err("%s line %d: rtp_num:%d f0:%d name:%s\n", __func__, __LINE__, sih_haptic->rtp.rtp_file_num, f0_file_num, aw8697_old_steady_test_rtp_name_0815[f0_file_num]);
		} else if (DEVICE_ID_1419 == sih_haptic->device_id) {
			ret = request_firmware(&rtp_file,
					aw8697_old_steady_test_rtp_name_1419[f0_file_num],
					sih_haptic->dev);
			hp_err("%s line %d: rtp_num:%d f0:%d name:%s\n", __func__, __LINE__, sih_haptic->rtp.rtp_file_num, f0_file_num, aw8697_old_steady_test_rtp_name_1419[f0_file_num]);
		}
	} else {
		if (DEVICE_ID_0815 == sih_haptic->device_id) {
			ret = request_firmware(&rtp_file,
					...
					sih_haptic->dev);
			hp_err("%s line %d: rtp_num:%d f0:%d name:%s\n", __func__, __LINE__, sih_haptic->rtp.rtp_file_num, f0_file_num, aw8697_high_temp_high_humidity_0815[f0_file_num]);
		} else if (DEVICE_ID_1419 == sih_haptic->device_id) {
			ret = request_firmware(&rtp_file,
...
static int ram_update(sih_haptic_t *sih_haptic)
{
	...
	if (DEVICE_ID_0815 == sih_haptic->device_id) {
		if (sih_haptic->detect.tracking_f0 < F0_VAL_MIN_0815 || sih_haptic->detect.tracking_f0 > F0_VAL_MAX_0815) {
			sih_haptic->detect.tracking_f0 = 1700;
		}
	} else if (DEVICE_ID_1419 == sih_haptic->device_id) {
		if (sih_haptic->detect.tracking_f0 < F0_VAL_MIN_1419 || sih_haptic->detect.tracking_f0 > F0_VAL_MAX_1419) {
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (((val >=  RINGTONES_START_INDEX && val <= RINGTONES_END_INDEX)
		...
		hp_err("%s:audio[%d] and haptic[%d]\n", __func__, sih_haptic->rtp.audio_ready, sih_haptic->rtp.haptic_ready);
	...
	if (!val && !(sih_haptic->rtp.rtp_file_num >= INUTP_LOW && sih_haptic->rtp.rtp_file_num <= INPUT_HIGH)) {
...
static void rtp_work_func(struct work_struct *work)
{
	...
	if (!rtp_file) {
		...
		hp_err("%s line:%d: rtp_num:%d name:%s\n", __func__, __LINE__, sih_haptic->rtp.rtp_file_num, rtp_name);
		vfree(rtp_name);
        if (ret < 0) {
